	...
	do
	{
		while (current.size()) {
			...
			if (map_e[current.front().first][current.front().second] == 0) {
				...
				if (map.size() > current.front().first + 1)
					if (map[current.front().first + 1][current.front().second] != 'T')
						next.push({ current.front().first + 1, current.front().second });

				if (-1 < current.front().first - 1)
					if (map[current.front().first - 1][current.front().second] != 'T')
						next.push({ current.front().first - 1, current.front().second });


				if (map[0].size() > current.front().second + 1)
					if (map[current.front().first][current.front().second + 1] != 'T')
						next.push({ current.front().first, current.front().second + 1 });

				if (-1 < current.front().second - 1)
					if (map[current.front().first][current.front().second - 1] != 'T')
						next.push({ current.front().first, current.front().second - 1 });
